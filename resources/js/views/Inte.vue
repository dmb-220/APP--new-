  ...
  <div>
    <section class="section is-main-section">
      <card-component title="VALDYMAS" icon="account-multiple">
        ...
        <button class="button is-sark" @click="rinktis('4')">Pard. be pašto</button>
        <button class="button is-sark" @click="rinktis('3')">Pard. per paštas</button>
      ...
      <card-component v-if="isjungta" title="Apyvarta" icon="account-multiple"> 
        ...
        <div  id="printMe">
          ...
          <div class="has-text-centered is-size-5">
            <b>Internetinė parduotuvė ( INTE )</b>
          </div>
          ...
          <div v-if="rodo == 4" class="has-text-centered is-size-5">Pardavimai (be pašto)</div>
          <div v-if="rodo == 3" class="has-text-centered is-size-5">Pardavimai (per paštą)</div>
          <div class="has-text-centered">
            LIETUVA
          </div>
          <br>
          ...
          default-sort="saskaitos_nr">
            ...
            <b-table-column class="has-text-right" label="Prestahop"  field="unikalus" v-slot="props">
                  {{ props.row.unikalus }}
            </b-table-column>
            ...
            <b-table-column :visible="rodo == 1 || rodo == 3 || rodo == 4" class="has-text-right" label="Pristatymas"  field="pristatymas" v-slot="props">
                  {{ parseFloat(props.row.pristatymas).toFixed(2) }}
            </b-table-column>
            <b-table-column :visible="rodo == 1 || rodo == 3 || rodo == 4" class="has-text-right" label="Per banka, Eur"  field="" v-slot="props">
          ...
          <template #footer>
                ...
                <th v-if="rodo == 1 || rodo == 3 || rodo == 4" class="has-text-right">{{ parseFloat(apy_pristatymas).toFixed(2) }}</th>
                <th v-else class="has-text-right"> </th>

                <th v-if="rodo == 1 || rodo == 3 || rodo == 4" class="has-text-right">{{ parseFloat(apy_suma+apy_pristatymas).toFixed(2) }}</th>
                <th v-else class="has-text-right"> </th>
            </template>
        </b-table>
        </div>
...
export default {
  ...
  rodyti(){
    ...
    if(this.rodo == 3){ this.data_list = this.pastas;}
    if(this.rodo == 4){ this.data_list = this.bepasto;}
  },
